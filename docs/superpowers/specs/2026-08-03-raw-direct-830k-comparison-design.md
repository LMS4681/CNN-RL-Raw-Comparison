...
## Budget: Stop at the Matched Step Count

The arm stops on a timestep ceiling of 830,000, the largest checkpoint-interval
multiple at or below the v7 final timestep of 833,368. It does not run a
wall-clock budget: the question being answered is step-for-step, and
equal-wall-clock efficiency is explicitly not of interest, so filling six hours
would only burn compute past the matched point.

The cost is receipts. `train.py` writes `run_state.json`,
`progress_timing.csv`, `runtime_metrics.json`, and `training_completion.json`
only under a wall-clock budget, and it rejects a run that reaches the timestep
ceiling while a wall-clock budget is active. The marker-validated staged
pipeline (`evaluate_arm_artifacts`, `validate_arm_evaluation_stage`,
`report_builder.write_complete_report`) requires those receipts through
`resolve_final_checkpoint`, so it does not apply to this arm. The substantive
comparison does not need them: `select_common_timestep`,
`readable_checkpoint_inventory`, and `evaluate_checkpoint` read only the
checkpoint archives and the `run_config.json` beside them.

Two numbers the receipts would have supplied are recovered elsewhere: parameter
counts by loading the saved model, and throughput from `arm_timing.json`, which
the notebook writes per session.

`notebooks/raw_direct_6h.ipynb` and the `raw-direct-6h-v1` tag preserve the
wall-clock variant of this arm for the case where the staged pipeline is
wanted. It is not the arm being run.

Step matching does not depend on the stopping rule, because arms are compared
at a common checkpoint. `WallClockBudgetCallback._on_step` checkpoints when
...
require. `configs/raw_direct_6h_seed0.json` exists for the wall-clock variant;
the step-bounded arm does not need it.

`--timesteps` is additive whenever a resumable archive is found, because
`model.learn` runs with `reset_num_timesteps=False`. The notebook therefore
reads the durable timestep and launches only `830,000 - done`, so interrupted
sessions converge on the target instead of adding it again:
...
`notebooks/raw_direct_830k.ipynb` has nine cells: title, Drive roots under
`/content/drive/MyDrive/CNN-RL-improved/raw-direct-830k-seed0` with
`TARGET_TIMESTEPS`, runtime guard, pinned checkout, hash-locked dependency
install, immutable input hashes, curve-log prune, training, and artifacts.

The training cell computes the remaining budget, streams child stdout, reports
progress every thirty seconds from the checkpoint filenames
(`block_placement_ppo_<steps>_steps.sb3`, since this arm has no
`run_state.json`), and appends a session record to `arm_timing.json`.

The Drive root already holds a partial run of 40,000 timesteps from the same
configuration. It is resumed rather than discarded: the remaining-budget
computation makes resuming the correct behaviour, and the arm still starts from
zero overall, as v7 did.

...
When present, `training_log.csv`, `loss_log.csv`, and `progress_timing.csv` are strict canonical inputs, not best-effort hints. Their headers and finite, monotonic rows (including progress generation, timestep, and recorded seconds) are validated; the learning plot shows terminal score, checkpoint wall-time progress, and the recorded PPO loss trace. An absent optional curve is labelled `자료 없음`.
